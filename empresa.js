...
let user=null,agents=[],tasks=[],approvals=[],events=[],leads=[],selectedSector="direcao";
...
async function boot(){
 ...
 await seedAgents();wireBrain();await refreshAll();selectSector("direcao");
}

async function refreshAll(){
 const [t,a,e,l,g]=await Promise.all([
 ...
 tasks=t.data||[];approvals=a.data||[];events=e.data||[];leads=l.data||[];agents=g.data||agents;
 renderMetrics();renderTasks();renderApprovals();renderEvents();renderAgents();fillAgentSelect();
}
...
window.createTask=async()=>{
 ...
 const {data,error}=await sb.from("business_tasks").insert({user_id:user.id,agent_id:agentId,sector:taskSector.value,title,description:taskDesc.value.trim(),priority:taskPriority.value,status:"todo"}).select().single();
